...
<div>
	...
	<div id="nav">
		<router-link to="/" @click="runningTest = false">Документация пользователя</router-link> |
		<router-link to="/test/train" @click="startTest(true)">Тренировочный ружим</router-link> |
...
export default {
	...
	methods: {
		...
		async startTest(train) {
			...
			let trainQC = null;
			if (train)
				trainQC = prompt('Введите количество вопросов каждого типа (4) в формате: n1; n2; n3; n4').split(';').map(v => v.replaceAll(' ', ''));
...
input[type="button"]:hover, input[type="submit"]:hover, button:hover {
	background: rgba(255, 65, 223, 0.7);
}
option {
